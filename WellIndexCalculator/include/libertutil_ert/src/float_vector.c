...
/**
   ...
   is charactereized by the following:
    ...
    o The content is only fundamental scalar types, like float, int
...
static float_vector_type * float_vector_alloc__(int init_size , float default_value, float * data, int alloc_size , bool data_owner ) {
  float_vector_type * vector = util_malloc( sizeof * vector );
...
float * float_vector_alloc_data_copy( const float_vector_type * vector ) {
  ...
  float * copy = util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
bool float_vector_init_range(float_vector_type * vector , float min_value , float max_value , float delta) {
  if (max_value >= min_value) {
    ...
    float_vector_append( vector , max_value );
    return true;
  } else
    return false;
}
...
/**
...

   {
      ...
      float_vector_permute( v3 , sort_perm );
      free(sort_perm);
   }
...
static int * float_vector_alloc_sort_perm__(const float_vector_type * vector, bool reverse) {
...
void float_vector_permute(float_vector_type * vector , const int * perm) {
  ...
  {
    int i;
    float * tmp = util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)

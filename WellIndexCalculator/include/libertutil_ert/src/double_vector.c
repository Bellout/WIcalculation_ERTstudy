...
static double_vector_type * double_vector_alloc__(int init_size , double default_value, double * data, int alloc_size , bool data_owner ) {
  double_vector_type * vector = util_malloc( sizeof * vector );
...
double * double_vector_alloc_data_copy( const double_vector_type * vector ) {
  ...
  double * copy = util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
bool double_vector_init_range(double_vector_type * vector , double min_value , double max_value , double delta) {
  if (max_value >= min_value) {
    ...
    double_vector_append( vector , max_value );
    return true;
  } else
    return false;
}
...
static int * double_vector_alloc_sort_perm__(const double_vector_type * vector, bool reverse) {
...
void double_vector_permute(double_vector_type * vector , const int * perm) {
  ...
  {
    int i;
    double * tmp = util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)

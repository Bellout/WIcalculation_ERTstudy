...
/**
   ...
   is charactereized by the following:
    ...
    o The content is only fundamental scalar types, like size_t, int
...
static size_t_vector_type * size_t_vector_alloc__(int init_size , size_t default_value, size_t * data, int alloc_size , bool data_owner ) {
  size_t_vector_type * vector = util_malloc( sizeof * vector );
...
size_t * size_t_vector_alloc_data_copy( const size_t_vector_type * vector ) {
  ...
  size_t * copy = util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
bool size_t_vector_init_range(size_t_vector_type * vector , size_t min_value , size_t max_value , size_t delta) {
  if (max_value >= min_value) {
    size_t current_value = min_value;
    size_t_vector_reset( vector );
    while (current_value < max_value) {
      size_t_vector_append( vector , current_value );
      current_value += delta;
    }
    size_t_vector_append( vector , max_value );
    return true;
  } else
    return false;
}
...
/**
...

   {
      ...
      size_t_vector_permute( v3 , sort_perm );
      free(sort_perm);
   }
...
static int * size_t_vector_alloc_sort_perm__(const size_t_vector_type * vector, bool reverse) {
...
void size_t_vector_permute(size_t_vector_type * vector , const int * perm) {
  ...
  {
    int i;
    size_t * tmp = util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)

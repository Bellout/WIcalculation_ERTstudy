...
/**
   ...
   is charactereized by the following:
    ...
    o The content is only fundamental scalar types, like bool, int
...
static bool_vector_type * bool_vector_alloc__(int init_size , bool default_value, bool * data, int alloc_size , bool data_owner ) {
  bool_vector_type * vector = util_malloc( sizeof * vector );
...
bool * bool_vector_alloc_data_copy( const bool_vector_type * vector ) {
  ...
  bool * copy = util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
bool bool_vector_init_range(bool_vector_type * vector , bool min_value , bool max_value , bool delta) {
  if (max_value >= min_value) {
    bool current_value = min_value;
    bool_vector_reset( vector );
    while (current_value < max_value) {
      bool_vector_append( vector , current_value );
      current_value += delta;
    }
    bool_vector_append( vector , max_value );
    return true;
  } else
    return false;
}
...
/**
...

   {
      ...
      bool_vector_permute( v3 , sort_perm );
      free(sort_perm);
   }
...
static int * bool_vector_alloc_sort_perm__(const bool_vector_type * vector, bool reverse) {
...
void bool_vector_permute(bool_vector_type * vector , const int * perm) {
  ...
  {
    int i;
    bool * tmp = util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)

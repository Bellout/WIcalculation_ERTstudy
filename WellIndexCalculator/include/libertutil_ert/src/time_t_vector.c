...
/**
   ...
   is charactereized by the following:
    ...
    o The content is only fundamental scalar types, like time_t, int
...
static time_t_vector_type * time_t_vector_alloc__(int init_size , time_t default_value, time_t * data, int alloc_size , bool data_owner ) {
  time_t_vector_type * vector = util_malloc( sizeof * vector );
...
time_t * time_t_vector_alloc_data_copy( const time_t_vector_type * vector ) {
  ...
  time_t * copy = util_calloc(vector->size , sizeof * copy );
  if (copy != NULL)
...
bool time_t_vector_init_range(time_t_vector_type * vector , time_t min_value , time_t max_value , time_t delta) {
  if (max_value >= min_value) {
    time_t current_value = min_value;
    time_t_vector_reset( vector );
    while (current_value < max_value) {
      time_t_vector_append( vector , current_value );
      current_value += delta;
    }
    time_t_vector_append( vector , max_value );
    return true;
  } else
    return false;
}
...
/**
...

   {
      ...
      time_t_vector_permute( v3 , sort_perm );
      free(sort_perm);
   }
...
static int * time_t_vector_alloc_sort_perm__(const time_t_vector_type * vector, bool reverse) {
...
void time_t_vector_permute(time_t_vector_type * vector , const int * perm) {
  ...
  {
    int i;
    time_t * tmp = util_alloc_copy( vector->data , sizeof * tmp * vector->size );
    for (i=0; i < vector->size; i++)
